...
import scripts.MatchFeatures as snn
...
class SIFTnnLabeler:
    ...
    def __init__(self, working_dir: str = '.'):
        ...
        # signal to stop running and save the current state of our model or data)
        self.__stop_running__ = False
    ...
    def labelFromVideo(self, path: str = '/dev/video1'):
        ...
        seq = 0
        itr = 0
        while cap.isOpened() and not self.__stop_running__:
    ...
    def matchFeatures(self, img1, img2):
        ...
        print(f'Labeling unmatched descriptors')
        unmatched_des1 = []
        unmatched_des2 = []
        ...
        #   label them as false
        min = 0
        if len(matches) < len(unmatched_des1) and len(matches) < len(unmatched_des2):
            min = int(len(matches)/2)
        elif len(unmatched_des1) < len(unmatched_des2):
            min = int(len(unmatched_des1)/2)
        else:
            min = int(len(unmatched_des2)/2)
        ...
        for m in good_matches:
            ...
            # Pair a good descriptor with a bad descriptor from each set
            self.x.append(des1[m.queryIdx] + random.choice(unmatched_des2))
            self.y.append(0.0)

            self.x.append(des2[m.trainIdx] + random.choice(unmatched_des1))
            self.y.append(0.0)
            ...
            while True:
                ...
                if match == m:
                    ...
                    self.x.append(des1[m.queryIdx] + des2[match.trainIdx])
                    self.x.append(des1[match.queryIdx] + des2[m.trainIdx])
                    self.y.append(0.0)
                    self.y.append(0.0)
                    break
    ...
    def labelAndTrain(self, path, model=None, video=False):
        ...
        optimizer = optim.SGD(self.__model__.parameters(), lr=0.01, momentum=0.9)
        criterion = nn.MSELoss() # binary cross entropy loss function
        ...
        seq = 0
        itr = 0
        while (len(images) == 2 or video) and not self.__stop_running__:
    ...
    def __train_model(self, **kwargs):
        ...
        x = torch.from_numpy(np.array(self.x))
        ...
        for x_batch, y_batch in train_loader:
            for i, x in enumerate(x_batch):
                # Stop training if signaled
                if self.__stop_running__:
    ...
    def __save_model(self, path: str='model.pth'):

        torch.save(self.__model__.state_dict(), path)
        print(f'Saved model as {os.path.abspath(path)}')

    def stopRunning(self):
        print('Signal recieved, shutting down labeler')
        self.__stop_running__ = True
...
if __name__ == '__main__':
    ...
    # Register our signal hyandler with `SIGINT`(ctrl +c)
    signal.signal(signal.SIGINT, signal_handler)

    lblr = SIFTnnLabeler('.')
    # lblr.labelFromVideo('../videos/drone.mp4')
    lblr.labelAndTrain('../videos/IMG_0173.mov', video=True)